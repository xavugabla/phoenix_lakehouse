"""
Consolidation task for CENACE PSC (Precios de Servicios Conexos) data.
...
@task(name="consolidate_cenace_psc", retries=2, log_prints=True)
def consolidate_cenace_psc(
    staged_partitions: List[str],
    config_path: Optional[str] = None
) -> Dict[str, int]:
    """
    Consolidates staged PSC data and appends it to the 'psc' Iceberg table.
    ...
    logger.info(f"Consolidating {len(staged_partitions)} staged PSC partitions.")
    ...
    try:
        ...
        table_identifier = f"{namespace}.psc"
        ...
        table_location = f"{base_location}/psc"
        ...
        publish_table_metadata("psc", table_location)

        return {"rows_appended": len(combined_df)}

    except Exception as e:
        logger.error("Failed during PSC consolidation and Iceberg append", exc_info=e)
        raise
